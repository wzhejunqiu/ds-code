...
func TestChatBlocksFromMessages(t *testing.T) {
	msgs := []session.Message{
		...
		{Role: "tool", Content: "<tool_result name=\"read_file\" id=\"1\">\noutput\n</tool_result>", ToolName: "read_file", ToolCallID: "1"},
		{Role: "system", Content: "rewound"},
		{Role: "assistant", Content: "", ToolCallsJSON: `[{"id":"1","name":"read_file","arguments":"{\"path\":\"a.go\"}"}]`},
		{Role: "tool", Content: "<tool_result name=\"read_file\" id=\"1\">\nmore\n</tool_result>", ToolName: "read_file", ToolCallID: "1"},
	}
	...
	if blocks[0].role != "user" || blocks[0].content.String() != "hello" {
		t.Fatalf("user block: %+v", blocks[0])
	}
	if blocks[1].role != "assistant" || blocks[1].content.String() != "hi" || blocks[1].reasoning.String() != "think" {
		t.Fatalf("assistant block: %+v", blocks[1])
	}
	if !blocks[1].reasoningOpen {
		t.Fatal("expected reasoning open")
	}
	if blocks[2].role != "tool" || blocks[2].toolName != "read_file" || blocks[2].toolResult != "more" {
...
func TestChatBlocksFromMessages_reasoningBeforeTools(t *testing.T) {
	msgs := []session.Message{
		{Role: "assistant", ReasoningContent: "think first", ToolCallsJSON: `[{"id":"1","name":"read_file","arguments":"{\"path\":\"a.go\"}"}]`},
		{Role: "tool", Content: "body", ToolName: "read_file", ToolCallID: "1"},
	}
	blocks := chatBlocksFromMessages(msgs, true)
	if len(blocks) != 2 {
		t.Fatalf("got %d blocks, want 2", len(blocks))
	}
	if blocks[0].role != "assistant" || blocks[0].reasoning.String() != "think first" {
		t.Fatalf("assistant block first: %+v", blocks[0])
	}
	if blocks[1].role != "tool" {
...
func TestChatBlocksFromMessages_durations(t *testing.T) {
	msgs := []session.Message{
		{Role: "assistant", Content: "hi", ReasoningContent: "think", ReasoningDurationMS: 1200, TurnDurationMS: 5000},
	}
