...
按 glob 模式在工作区内查找**文件**路径（不含目录），返回相对项目根的路径列表。自动跳过常见二进制文件（扩展名、`mimetype` magic、NUL 字节启发式）。
...
[`globmatch.MatchFiles`](../../globmatch/globmatch.go) 统一处理 `**` 与 `filepath.Glob`。
...
3. [`textfile.IsSearchable`](../../textfile/textfile.go) 跳过 PNG/PDF/ZIP 等二进制。
4. 计数达到 `max_results` 后截断。

含 `**` 时在 `MatchFiles` 阶段预取 `limit*4` 条候选再过滤，以平衡性能与结果量。
