...
func (t *TaskTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if !ok || inv.SessionID == "" || inv.ToolCallID == "" {
		return "", fmt.Errorf("task: missing parent session or tool call id")
	}
	if t.Subagent == nil {
		return "", fmt.Errorf("task: subagent store not configured")
	}
	...
	summary, runErr := subagent.Run(ctx, t.Cfg, t.LLM, in.Prompt, func(reg *toolpkg.Registry) {
