...
本版**仅交付设计文档**。下列验收标准分两类：

- **DOC 验收**：本版即可核验——文档完整性、决策一致性、契约不破坏。
- **IMPL 验收（AC-x）**：面向后续实现里程碑（M0–M4）的功能验收标准，作为实现期回归依据；本版不要求代码通过。
...
| ID     | 标准                                                                                              |
| ------ | ----------------------------------------------------------------------------------------------- |
| DOC-1  | `docs/v0.2.0/` 含 README / REQUIREMENTS / DESIGN / ACCEPTANCE 四件套，互相链接正确                |
...
| DOC-5  | `docs/README.md` v0.2.0 行更新；[DESKTOP.md](../DESKTOP.md) 顶部标记 deprecated 并指向本版         |
...
| 里程碑 | 必过验收                                                              |
| ------ | ------------------------------------------------------------------- |
| M0     | AC-11.1/11.2/11.3、AC-7.1、AC-6.1（最小审批）、**AC-12.5**（内嵌 Chromium PoC）、**AC-13**（Bun 工具链） |
| M1     | AC-1/2/3/4/5、AC-6、AC-7、AC-10、AC-11、REG-1~4、AC-12.1/12.4        |
| M2     | AC-8、AC-9、AC-12.2/12.3、命令面板 / Plan / 子代理                   |
| M3     | checkpoint/rewind、MCP/LSP 管理、billing、托盘、自动更新             |
| M4     | 助手 HTML 输出模式（OWASP XSS 全通过 + 流式无卡顿）                  |
