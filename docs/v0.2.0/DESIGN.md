...
v0.2.0 验证总纲中以下技术决策的可行性，UI 可为**最小单栏聊天**，不实现三栏与 workspace 管理。

完整架构、协议、目录结构见总纲；本节仅列**本版必须落地的模块**。
...
- 本版 `workspaceId` 可固定为 `"default"`（多 workspace 路由 v0.2.1 补齐）
...
v0.2.1 在此基础上添加 `WorkspaceManager`、三栏 UI、完整 P0 功能，**不重构** bridge 协议与 datadir 契约。
