...
> 本版仅**文档**；下表为后续实现建议分期，落到 v0.2.1+ 等小版本。

| 里程碑 | 目标                                                                                                       | gate                                   |
| ------ | -------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| **M0** | 技术验证：Wails v3 工程 + **Bun + Vite 工具链 PoC** + 复用 `newRunner` 跑通一轮 `RunTurn` + StreamEmitter 流式文本 + 一次审批 + 取消 | 能打开项目、发消息、看流式、批准一次写 |
| **M1** | MVP：多 workspace + 会话 + 三栏聊天 + 工具卡片 + 内联审批 + 停止按钮 + 设置视图 + onboarding + 打包       | FR-0/1/2/3/4/5/6/7/14/15/17（P0）绿    |
| **M2** | 对齐增强：Inspector（Monaco diff）+ Plan + 子代理 + 命令面板 + 系统集成 + 签名公证                        | FR-8/9/10/11/13.4/16/18.2/18.3         |
| **M3** | 深化：checkpoint/rewind + MCP/LSP 管理 + billing + 托盘 + 会话搜索 + 自动更新                             | FR-12/13/16.5/2.5/18.4                 |
| **M4** | 可选：助手 HTML 输出模式（独立安全 PoC 门禁通过后）                                                       | FR-4.8；OWASP XSS 全通过               |
